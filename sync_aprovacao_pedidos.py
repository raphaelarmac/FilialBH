"""
sync_aprovacao_pedidos.py
=========================
Lê no SAP a tabela bi_bs_fluxo_aprov (fluxo de aprovação dos pedidos de compra)
e envia 1 linha por pedido (a aprovação mais recente) para o webhook do app:

  POST {APP_BASE_URL}/api/public/hooks/sync-aprovacao-pedidos
...
# Mesma logica do Power BI: ordena pela aprovacao mais recente e faz distinct por pedido.
QUERY = """
SELECT DISTINCT
    ...
    CASE
        WHEN "StatusFluxoAprovacao" = 'STARTED'   THEN '1. Iniciado/Aguardando'
        WHEN "StatusFluxoAprovacao" = 'READY'     THEN '2. Pronto para Aprovacao'
        WHEN "StatusFluxoAprovacao" = 'COMPLETED' THEN '3. Aprovado'
        WHEN "StatusFluxoAprovacao" = 'CANCELLED' THEN '0. Cancelado/Reprovado'
...
ORDER BY hora_inicio DESC, hora_fim DESC, data_inicio DESC
"""
...
def post_webhook(payload: dict) -> None:
    url = f"{APP_BASE_URL}/api/public/hooks/sync-aprovacao-pedidos"
...
def main() -> None:
    ...
    # distinct por pedido mantendo a primeira ocorrencia (ja vem ordenado do mais recente)
    dedup: dict[str, dict] = {}
    for r in rows:
        pedido = str(r.get("pedido") or "").strip().lstrip("0")
        if not pedido or pedido in dedup:
            continue
        item = {k: to_jsonable(v) for k, v in r.items()}
        item["pedido"] = pedido
        dedup[pedido] = item
